...
class InjectTest < Minitest::Test
  ...
  def test_multiple_some_numbers
    assert_equal 151200, (5..10).inject(1, :*)
  end

  def test_multiple_some_numbers_with_block
    assert_equal 151200, (5..10).inject(1) {|product, n| product * n}
  end

  def test_finding_the_longest_word
    assert_equal 'sheep', %w(cat sheep bear).inject {|memo, word| memo.length > word.length ? memo : word}
  end
end
